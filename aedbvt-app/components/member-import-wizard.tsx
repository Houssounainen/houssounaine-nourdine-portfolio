...
function normalizeHeader(value:string){
  return value.trim().toLocaleLowerCase("fr").split(" ").filter(Boolean).join(" ");
}

function parseCsv(text:string){
  const newlineIndex=text.indexOf("\n");
  const firstLine=(newlineIndex>=0?text.slice(0,newlineIndex):text).replace(/\r$/,"");
  const delimiters=[";",",","\t"];
...
function validateRow(row:ImportRow,line:number):ParsedRow{
  ...
  if(row.email&&!validEmail(row.email.trim())) errors.push("Email invalide");
  if(row.joined_at&&!validIsoDate(row.joined_at.trim())) errors.push("Date attendue : YYYY-MM-DD");
...
export function MemberImportWizard(){
  ...
  async function readFile(file:File){
    ...
    if(matrix.length<2){
      setRows([]);
      setError("Le fichier doit contenir une ligne d’en-têtes et au moins un membre.");
      return;
    }
    ...
    if(!keys.includes("full_name")||!keys.includes("village")){
      setRows([]);
      setError("Colonnes obligatoires absentes : nom complet et village.");
      return;
    }
    ...
    const parsed=matrix.slice(1).map((values,index)=>{
      ...
      return validateRow(row as ImportRow,index+2);
    });
    setRows(parsed);
  }
  ...
  function downloadTemplate(){
    ...
    anchor.href=url;
    anchor.download="modele-import-membres-aedbvt.csv";
    anchor.click();
  ...
  return <div className="import-wizard">
    <div className="panel import-controls">
      <div>
        ...
        <p>Les colonnes obligatoires sont <b>nom complet</b> et <b>village</b>. Aucun membre existant ne sera écrasé.</p>
      </div>

      <div className="import-actions">
        <label className="button primary import-file">
          Choisir un CSV
          <input type="file" accept=".csv,text/csv" onChange={(event)=>event.target.files?.[0]&&readFile(event.target.files[0])}/>
        </label>
        <button className="button secondary" type="button" onClick={downloadTemplate}>Télécharger le modèle</button>
      </div>
    ...
    {rows.length>0&&<div className="panel">
      <div className="panel-head">
        <div><span className="eyebrow">Étape 2</span><h2>Prévisualisation</h2></div>
        <span className="status-pill">{valid.length} valide(s) · {invalid} erreur(s)</span>
      </div>

      <div className="table-wrap">
        <table>
          <thead><tr><th>Ligne</th><th>Nom</th><th>Village</th><th>Filière</th><th>Email</th><th>Statut</th><th>Validation</th></tr></thead>
          <tbody>
            {rows.slice(0,100).map((row)=><tr key={row._line}>
              <td>{row._line}</td>
              ...
              <td>{row._errors.length
                ?<span className="badge import-invalid">{row._errors.join(" · ")}</span>
                :<span className="badge ok">Valide</span>}</td>
            </tr>)}
          </tbody>
        </table>
      </div>
      ...
      <div className="import-submit">
        <button className="button primary" disabled={busy||invalid>0||!valid.length} onClick={submit}>
          {busy?"Import en cours…":"Importer "+valid.length+" membre(s)"}
        </button>
        {invalid>0&&<small>Corrigez les lignes invalides avant l’import.</small>}
      </div>
    </div>}
    ...
    {result&&<div className="panel import-result">
      ...
      <div className="import-result-stats">
        <span><b>{result.inserted}</b><small>ajouté(s)</small></span>
        <span><b>{result.skipped}</b><small>doublon(s) ignoré(s)</small></span>
        <span><b>{result.invalid}</b><small>invalide(s)</small></span>
      </div>
