...
export default async function DashboardPage() {
  ...
  const [members, payments, events, meetings, docs, settings, articles, announcements] = await Promise.all([
    ...
    <section className="page">
      ...
      {pinned.length > 0 && (
        <div className="dashboard-alerts">
          {pinned.slice(0,2).map((item)=><Link href="/announcements" className={"dashboard-alert level-"+item.level} key={item.id}><b>{item.level === "urgent" ? "Urgent" : "Annonce"}</b><span>{item.title}</span><small>{item.message}</small></Link>)}
        </div>
      )}
      ...
      <div className="content-grid dashboard-grid">
        <article className="panel">
          <div className="panel-head"><div><span className="eyebrow">Agenda</span><h2>Prochains rendez-vous</h2></div><Link href="/agenda">Voir tout →</Link></div>
          <div className="mini-feed">
            {agenda.map((item)=><div key={item.kind+"-"+item.id}><span className="mini-date">{new Date(item.starts_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short"})}</span><div><b>{item.title}</b><small>{item.kind} · {item.location || "Lieu à confirmer"}</small></div></div>)}
            {!agenda.length && <p>Aucun rendez-vous planifié.</p>}
          </div>
        </article>

        <article className="panel">
          <div className="panel-head"><div><span className="eyebrow">À la une</span><h2>Dernières actualités</h2></div><Link href="/news">Voir tout →</Link></div>
          <div className="mini-feed">
            {(articles.data || []).map((item)=><div key={item.id}><span className="badge">{item.category || "Actualité"}</span><div><b>{item.title}</b><small>{item.excerpt || "Publication AEDBVT"}</small></div></div>)}
            {!articles.data?.length && <p>Aucune actualité publiée.</p>}
          </div>
        </article>
      </div>

      <div className="quick-links">
        <Link className="quick-card" href="/members"><span>Membres</span><b>Registre & adhésions</b><small>Numéros, filières, rôles et accès.</small></Link>
        <Link className="quick-card" href="/finance"><span>Finances</span><b>Cotisations & caisse</b><small>Paiements, dépenses et reçus.</small></Link>
        <Link className="quick-card" href="/organization"><span>Organisation</span><b>Bureau & missions</b><small>Organigramme et titulaires.</small></Link>
        <Link className="quick-card" href="/governance"><span>Gouvernance</span><b>Textes & transparence</b><small>Statuts, règlement et documents.</small></Link>
      </div>
    </section>
  );
}
