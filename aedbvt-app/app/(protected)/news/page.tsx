...
export default async function NewsPage({
  ...
  const {data:rows}=await supabase.from("articles")
    .select("id,title,slug,excerpt,body,category,published_at,created_at,featured")
    .eq("published",true)
    .eq("status","published")
  ...
  const articles=(rows||[])
    ...
    .sort((a,b)=>Number(b.featured)-Number(a.featured)||new Date(b.published_at||b.created_at).getTime()-new Date(a.published_at||a.created_at).getTime());
  ...
  return <section className="page">
    ...
    {featured.length>0&&<section className="featured-news">
      ...
      <div className="featured-news-grid">
        {featured.map((article)=><Link className="featured-news-card" href={"/news/"+article.slug} key={article.id}>
          <span className="badge featured">À la une</span>
          <small>{article.category||"Actualité"} · {new Date(article.published_at||article.created_at).toLocaleDateString("fr-FR")}</small>
          <h3>{article.title}</h3>
    ...
    <div className="article-grid">
      {regular.map((article)=><article className="panel article-card" key={article.id}>
